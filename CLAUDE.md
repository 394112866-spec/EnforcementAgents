...
| 输入框元数据 / 工作区文件 IO 走 sidecar HTTP（`/api/files/*`、`/agent/search-files`、`/api/commands`、`/agent/{file,check-paths,download,open-in-finder,open-with-default,open-path}`） | 启动页没有 Sidecar，这些路径在 launcher 直接死掉（PRD 0.2.7 实战：`'API 未就绪'` toast、空斜杠菜单）；并且把"AI runtime 容器"和"OS 文件操作"耦合在一起，未来云端协作把客户端拆出来时分不开 | 走 Rust invoke：`cmd_workspace_*`（见 `src-tauri/src/workspace_files/`）。前端入口 `useWorkspaceFileService(workspacePath)`；绝对路径揭示用 `cmd_open_path_external`（Skill/Command 详情面板）。Phase D.5（v0.2.7）已完成所有 renderer 调用迁移；Phase E 将删除 sidecar 端 handler |
